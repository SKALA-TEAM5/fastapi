...
# ※ 페이지 6 (안전보건교육비 등) — DB에 해당 카테고리 코드 없음 → None 처리
PAGE_CATEGORY_MAP = {
    1: None,        # 요약 페이지
    ...
    4: "CAT_03",    # 보호구 등
    5: "CAT_04",    # 안전보건진단비 등
    6: None,        # 안전보건교육비 등 — DB 미대응 (별도 경고 처리)
    7: "CAT_05",    # 근로자 건강장해예방비 등
    8: "CAT_06",    # 건설재해예방전문지도기관 기술지도비
    9: "CAT_07",    # 본사 전담조직 근로자 임금 등
    10: "CAT_08",   # 위험성평가 등에 따른 소요비용
}

# 페이지 번호별 스킵 사유 (None 으로 처리되는 페이지 중 경고가 필요한 경우)
PAGE_SKIP_REASONS = {
    6: "안전보건교육비 등 — DB(V4)에 해당 카테고리 코드 없음. 팀 확인 필요.",
}
...
CATEGORY_NAME_MAP = {
    ...
    "CAT_07": "본사 전담조직 근로자 임금 등",
    "CAT_08": "위험성평가 등에 따른 소요비용",
}

# 카테고리별 컬럼 구조 정의 (DB 카테고리 코드 기준 — CAT_01~CAT_08)
# ※ 안전보건교육비 등(구 코드 "5")은 DB 미대응이므로 제외
CATEGORY_COLUMNS = {
    "CAT_01": {
        "date":        ["지급일", "지급 일"],
    ...
    # CAT_05: 근로자 건강장해예방비 등 (공식 서식 6페이지)
    "CAT_05": {
        "date":        ["사용일", "사용일자"],
        "description": ["구 분", "구분"],

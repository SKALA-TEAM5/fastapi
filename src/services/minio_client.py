...
MinIO 파일 접근 유틸리티
━━━━━━━━━━━━━━━━━━━━━━
storage_key를 받아 MinIO에서 파일 바이트를 가져오거나
외부 Agent가 사용할 presigned URL을 생성한다.
...
def _get_client(endpoint_url: str | None = None):
    """MinIO 호환 boto3 클라이언트 생성 (환경변수 검증 포함)"""
    if not S3_BUCKET:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="S3_BUCKET 또는 APP_MINIO_BUCKET 환경변수가 설정되지 않았습니다.",
        )
    return boto3.client(
        "s3",
...
def fetch_file(storage_key: str) -> bytes:
    """
    MinIO에서 파일을 가져와 bytes로 반환한다.

    Args:
        storage_key: MinIO 오브젝트 키 (DB files.storage_key 값)
    ...
    Raises:
        ...
        HTTPException 503 — MinIO 자격증명 오류 / 네트워크 오류
    """
    client = _get_client()
    try:
        ...
        if code in ("NoSuchKey", "AccessDenied", "403", "404"):
            raise HTTPException(
                ...
                detail=f"MinIO 파일에 접근할 수 없습니다 (key={storage_key}, code={code})",
            )
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"MinIO 오류: {e}",
...
def create_presigned_file_url(
    ...
    """
    MinIO 오브젝트를 읽을 수 있는 presigned GET URL을 생성한다.
    ...
    Raises:
        ...
        HTTPException 503 — MinIO 설정 또는 자격증명 오류
    """
    if not storage_key:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail=f"MinIO presigned URL 생성 오류: {e}",
        )
    except BotoCoreError as e:
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,

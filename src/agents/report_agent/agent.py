...
class ReportAgent:
    ...
    def __init__(self, llm_client: LLMClient | None = None, *, use_default_llm: bool = True) -> None:
        ...
        if self.llm_client is None:
            if not use_default_llm:
                raise ReportLLMError("ReportAgent requires an LLM client because overall_opinion must be generated by LLM.")
            self.llm_client = OpenAIReportLLMClient.from_environment()
        if self.llm_client is None:
            raise ReportLLMError("OPENAI_API_KEY is required because overall_opinion must be generated by LLM.")

    def generate(self, context: ReportContext) -> ReportDraft:
        """보고서 초안을 만들고, LLM이 작성한 총평 문장을 병합합니다."""
        ...
        try:
            ...
            raise ReportLLMError(f"LLM report generation failed: {type(exc).__name__}") from exc
    ...
    def _merge_llm_text(self, draft: ReportDraft, payload: dict) -> ReportDraft:
        ...
        overall_opinion = payload.get("overall_opinion")
        if not isinstance(overall_opinion, str) or not overall_opinion.strip():
            raise ReportLLMError("LLM response must include a non-empty overall_opinion.")
        if len(overall_opinion.strip()) < MIN_LLM_OVERALL_OPINION_CHARS:
            raise ReportLLMError(
                f"LLM overall_opinion must be at least {MIN_LLM_OVERALL_OPINION_CHARS} characters for an audit report."
            )
